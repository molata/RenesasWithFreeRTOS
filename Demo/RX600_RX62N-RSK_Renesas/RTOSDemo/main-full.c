...
#define mainREG_TEST_1_PARAMETER	( 0x12121212UL )    // 通过这种定义的方式设置参数
#define mainREG_TEST_2_PARAMETER	( 0x12345678UL )
#define mainHIT_TEST_EASY_PARAMETER 10                  // 测试taskCreate的传入参数功能
/* Priorities at which the tasks are created. */
#define mainCHECK_TASK_PRIORITY		( configMAX_PRIORITIES - 1 )          // 常量要养成Define的好习惯
...
/*-----------------------------------------------------------*/
extern void prvHITTestTask( void *pvParameters );
...
extern void vHandlerTask( void *pvParameters );
#if mainTASK_IMPLEMENT == 0
	HardwareSetup(); 
	vSemaphoreCreateBinary( xBinarySemaphore );           // 信号量必须要在main函数里面创建
